...
public class KafkaConsumerService {
    ...
    public void consumeTransactionEvent(TransactionEvent event) {
        ...
        try {
            if ("COMPLETED".equals(event.getStatus())) {
                ...
                log.info("Skipping transaction event with status: {} (only COMPLETED transactions create notifications)", event.getStatus());
            }
        } catch (Exception e) {
    ...
    private void createNotificationForTransaction(TransactionEvent event) {
        ...
        try {
            ...
            notification.setType(Notification.NotificationType.EMAIL); // Usar EMAIL en lugar de DEFAULT (CHECK constraint)
            notification.setNotificationType(Notification.NotificationType.EMAIL); // Usar EMAIL en lugar de DEFAULT (CHECK constraint)
            notification.setStatus(Notification.NotificationStatus.PENDING);
            notification.setNotificationId("NOTIF-" + event.getTransactionId());
            
            // Establecer recipient (requerido por la base de datos)
            ...
            String subject = "Transaction " + (event.getTransactionType() != null ? event.getTransactionType() : "Unknown");
